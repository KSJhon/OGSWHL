...
step_algorithm=@(X,w,nu_r,mu_r,sigma_r,regularize,delta_r)studentT_step(X,w,nu_r,mu_r,sigma_r,regularize,@constant_nu,delta_r);
...
%                  [mu_r,Sigma_r,nu_r]=iterate_studentT(X,size(A,2),'GMMF',300,1);
                [mu_r,nu_r,Sigma_r]=iterate_studentT(X,size(A,2),step_algorithm,300,1);
    ...
    disp(char(['Estimated sigma = ', num2str(sqrt(mean(S)))]))
    disp(char(['geometric sigma = ', num2str(geomean(S))]))
    
end
function [nu_r_plus_one,evals]=constant_nu(~,~,~,~)
nu_r_plus_one=1;
evals=0;
end
